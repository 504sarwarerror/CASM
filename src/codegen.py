...
class CodeGenerator:
    ...
    def generate_if(self):
        ...
        self.output.append(f"    {jump_map[op.type]} {label_next}")
    ...
    def generate_while(self):
        ...
        self.output.append(f"    {jump_map[op.type]} {label_end}")
    ...
    def generate_call(self):
        ...
        if self.current_token() and self.current_token().type == TokenType.LPAREN:
            ...
            while self.current_token() and self.current_token().type in [
                TokenType.STRING, TokenType.IDENTIFIER, TokenType.NUMBER, TokenType.REGISTER
            ]:
                args.append(self.current_token())
                self.advance()
        ...
        if func_name == 'print':
            ...
            self.generate_print(args)
            self.output.append(f"    lea rcx, [rel _newline_str]")
            self.output.append(f"    call _print_string")
            self.stdlib_used.add('print')
        elif func_name == 'scan':
            self.generate_scan(args)
        elif func_name == 'scan_int':
            self.generate_scan_int(args)
        elif func_name in ['strlen', 'strcpy', 'strcmp', 'strcat', 'abs', 'min', 'max', 
                          'pow', 'array_sum', 'array_fill', 'array_copy', 'memset', 
                          'memcpy', 'rand', 'sleep']:
    ...
    def generate_scan_int(self, args):
        if not args:
            return
        var = args[0].value
        self.output.append(f"    lea rcx, [rel {var}]")
        self.output.append(f"    call _scan_int")
